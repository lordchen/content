...
function setPreviewActionMessage(text = "", tone = "") {
  ...
  if (!node) return;
  node.textContent = text;
  node.className = `v34-message v34-preview-action-message ${tone}`.trim();
}
  ...
  document.body.innerHTML = `
    <main class="v34-login">
      <section class="v34-card v34-login-card">
        <h1>登录内容中台</h1>
        <p class="v34-muted">登录后查看 v3.4 新版脚本库。</p>
        <form id="loginForm">
...
async function loadScripts() {
  setMessage("正在读取脚本库...", "");
  $("#scriptRows").innerHTML = `<tr><td colspan="7"><div class="v34-empty">正在读取脚本库...</div></td></tr>`;
  try {
    ...
    $("#scriptRows").innerHTML = `<tr><td colspan="8"><div class="v34-empty">${escapeHtml(error.message || "读取失败")}</div></td></tr>`;
    renderPreview(null);
...
function renderScripts() {
  ...
  if (!rows.length) {
    $("#scriptRows").innerHTML = `<tr><td colspan="8"><div class="v34-empty">暂无符合条件的脚本</div></td></tr>`;
    renderPreview(null);
    return;
  }
  if (!rows.some((item) => Number(item.id) === Number(state.selectedId))) state.selectedId = Number(rows[0].id);
...
function scriptRow(item) {
  ...
  return `
    <tr class="${selected}">
      ...
      <td><span class="v34-tag ${statusMeta.className}">${escapeHtml(statusMeta.label)}</span></td>
      <td><span class="v34-text-line">${escapeHtml(coreStructure(item))}</span></td>
      <td>${feishuSyncBadge(item)}</td>
      <td>${escapeHtml(formatDate(item.updatedAt || item.createdAt))}</td>
      <td>
        <span class="v34-row-actions">
          <button class="v34-link-btn" type="button" data-open-script="${id}">预览</button>
          <button class="v34-link-btn" type="button" data-review-script="${id}" data-review-status="adopted" ${syncDisabledAttr(item)}>${feishuSynced(item) ? "已同步" : "同步飞书"}</button>
          <button class="v34-link-btn" type="button" data-rewrite-script="${id}">重写</button>
        </span>
      </td>
    </tr>
  `;
...
function renderPreview(item) {
  ...
  if (!item) {
    body.innerHTML = `<div class="v34-empty">选择一条脚本后在这里预览。</div>`;
    actions.hidden = true;
  ...
  body.innerHTML = `
    <section class="v34-script-meta-stack">
      <div class="v34-script-meta-item">
        <span>脚本标题</span>
        <h4 class="v34-script-preview-title">${escapeHtml(item.title || "未命名脚本")}</h4>
      </div>
      <div class="v34-script-meta-item">
        <span>关联活动</span>
      ...
      <div class="v34-script-meta-item">
        <span>状态</span>
        <strong><span class="v34-tag ${meta.className}">${escapeHtml(meta.label)}</span></strong>
      </div>
      <div class="v34-script-meta-item">
        <span>飞书同步</span>
    ...
    <section class="v34-detail-block">
      <h4>风险提示</h4>
      <p class="v34-risk-box">${escapeHtml(riskText(item))}</p>
    </section>
  `;
...
function scriptBodyHtml(item) {
  ...
  if (!scenes.length) {
    return `<p class="v34-detail-text">${escapeHtml(item.scriptBody || item.hook || "暂无脚本内容")}</p>`;
  }
  return `
    <div class="v34-script-table" role="table" aria-label="分镜脚本">
      ...
      ${scenes.slice(0, 4).map((scene) => `
        <div class="v34-script-row" role="row">
          ...
          <span>
            <span class="v34-script-part"><b>镜头</b><em>${escapeHtml(scene.shot || "-")}</em></span>
            <span class="v34-script-part"><b>口播</b><em>${escapeHtml(scene.voiceover || "-")}</em></span>
            <span class="v34-script-part"><b>字幕</b><em>${escapeHtml(scene.subtitle || "-")}</em></span>
          </span>
        </div>
      `).join("")}
    </div>
...
async function confirmRewrite() {
  ...
  if (!feedback) {
    ...
    $("#rewriteMessage").className = "v34-message error";
    return;
  }
...
function feishuSyncBadge(item) {
  ...
  return `<span class="v34-tag ${meta.className}" title="${escapeHtml(meta.title)}">${escapeHtml(meta.label)}</span>`;
}
...
function campaignName(item) {
  const output = item.output || {};
  const snapshot = output.inputSnapshot || item.inputSnapshot || {};
