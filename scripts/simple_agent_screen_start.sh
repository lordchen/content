...
screen -dmS simple-agent-api bash -lc "cd '$ROOT_DIR' && if [ -f .env ]; then set -a; source .env; set +a; fi; exec .venv/bin/python scripts/prototype_api_server.py >> logs/simple-agent-api.log 2>&1"
